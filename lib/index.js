...
module.exports = {
      ...
      upload(file, customParams = {}) {
        return new Promise((resolve, reject) => {
          ...
          S3.upload(
            ...
            (err, data) => {
              if (err) {
                console.log('strapi-upload-swisscom-s3 upload err: ', err);
                return reject(err);
              }
      ...
      delete(file, customParams = {}) {
        return new Promise((resolve, reject) => {
          ...
          S3.deleteObject(
            ...
            (err, data) => {
              if (err) {
                console.log('strapi-upload-swisscom-s3 delete err: ', err);
                return reject(err);
              }
  ...
  createBucketIfNotExists(config) {
    ...
    S3.listBuckets((err, data) => {
      if (err) console.log('Error listBuckets', err);
      else {
        console.log('Existing bucket list', data);
        ...
        if (!bucket) {
          ...
          S3.createBucket({ Bucket: config.params.Bucket }, (err, data) => {
            if (err) console.log('Error createBucket:', err);
            else console.log('Success createBucket:', data);
          });
        }
